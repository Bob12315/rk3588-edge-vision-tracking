...
3. 直接点击一个英文目标，或手动输入其他目标，再用 YOLO-World 定位、所选跟踪器分配持续 ID。

也可以点击“扫描并选择人物”。该模式先用 `person` 连续检测18帧，只把稳定
跟踪轨迹交给 VLM 生成英文人物卡片；选择卡片后继续原跟踪会话并只显示该 ID。
...
连接后网页持续显示最新画面。VLM 分析使用点击时的当前帧；开始检测后，YOLO-World 与所选跟踪器处理后续画面。
...
VLM 负责理解和生成 grounding 计划，不负责逐帧画框。YOLO-World 负责定位，所选跟踪器负责跨帧 ID。衣服颜色会经过通用颜色分类和按 Track ID 的三帧稳定，支持 12 个标准颜色；其他关系属性目前只展示在计划中，不会被错误地宣称为已验证。详见[衣服颜色识别与跟踪](CLOTHING_COLOR_TRACKING.md)。

“跟踪模式”默认是 ByteTrack，适合实时预览；选择“身份增强”时使用
BoT-SORT + ReID，用模型原生外观特征辅助遮挡和多人交叉关联。该选项会降低帧率，
且不代表长时离画重入必然保持 ID。配置、实测与 RK3588 路径见
[身份增强跟踪](IDENTITY_TRACKING.md)。
...
- 切换目标会新建 YOLO-World/跟踪器实例，避免沿用上一个目标的轨迹状态。
