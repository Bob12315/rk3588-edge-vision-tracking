...
- `白衣服的人` → YOLO-World 提示词 `person`，并自动启用上半身 HSV 白衣复核。
...
VLM 负责理解和生成 grounding 计划，不负责逐帧画框。YOLO-World 负责定位，ByteTrack 负责跨帧 ID。除白衣 HSV 规则外，颜色和关系属性目前只会展示在计划中，不会被错误地宣称为已验证。
