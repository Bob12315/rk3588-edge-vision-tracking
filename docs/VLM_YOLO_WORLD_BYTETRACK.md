...
- 白衣任务使用 `person` 定位后的通用衣服颜色分类，并按 Track ID 做多帧稳定；结构化 VLM 结果声明 `white clothing` 是必要属性。
...
这段素材只有约 1.89 秒，而且不是专门的跟踪评测集。它证明了工程链路和 ID 数据可用，不证明已通过遮挡、人员交叉、离画再入和 ID-switch 指标。颜色复核位于 ByteTrack 之后，某些帧的跟踪框如果包含较多背景或旁人，可能不通过颜色阈值；因此不能把“该 ID 本帧未输出”解读为跟踪器丢失。
